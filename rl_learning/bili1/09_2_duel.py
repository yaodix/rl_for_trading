...
duel:对决
明确区分状态价值和动作优势
Dueling DQN: 分解为Q(s,a) = V(s) + A(s,a)
V(s): 状态s的内在价值（不管采取什么动作的平均回报）
A(s,a): 在状态s下采取动作a相比平均动作好多少
'''
...
class QNet_Duel(nn.Module):
    ...
    def forward(self, x):
        ...
        advantage = self.linear2(hidden) # 在状态s下采取动作a相比平均动作好多少
        value = self.linear3(hidden)  # 状态s的内在价值
        value = value.expand_as(advantage)
        
        # 计算最终Q值：Q(s,a) = V(s) + A(s,a) - mean(A(s,a'))
        # 这里减去优势的均值是为了使优势函数的平均值为0
        # 保证Q值的唯一性（避免V(s)和A(s,a)的表示冗余）
...
if __name__ == '__main__':
    ...
    train(env, 5000)  # Running episode  5000, step 192776 Reaches goal 9.00%. 