...
def setup_ignite(engine: Engine, exp_source, run_name: str,
    ...
    ptan_ignite.EpisodeFPSHandler().attach(engine)  # 计算每秒处理的帧数（训练速度）
    ...
    handler = tb_logger.OutputHandler(tag="episodes", metric_names=metrics)  # tag是TensorBoard 中的标签
    event = ptan_ignite.EpisodeEvents.EPISODE_COMPLETED
    # 当 event 发生时，自动记录 metrics 到 TensorBoard
    # 将 TensorBoard 日志处理器附加到 engine
    ...
    handler = tb_logger.OutputHandler(
        ...
        output_transform=lambda a: a)  # 输出转换函数（这里直接返回）
